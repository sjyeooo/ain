...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_defi --run_test=util_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_defi --run_test=util_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
...
BOOST_AUTO_TEST_CASE(test_FormatUserAgentString)
{
    ...
    BOOST_CHECK_EQUAL(FormatUserAgentString("Test", 99900, std::vector<std::string>()),std::string("/Test:0.9.99/"));
    BOOST_CHECK_EQUAL(FormatUserAgentString("Test", 99900, comments),std::string("/Test:0.9.99(comment1)/"));
    BOOST_CHECK_EQUAL(FormatUserAgentString("Test", 99900, comments2),std::string("/Test:0.9.99(comment1; Comment2; .,_?@-; )/"));
}
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("defi"), "Defi");
...
{
    auto testFunRes = []() {
        verifyRes(Res::Err("Error"));
        return Res::Ok();
    };
    auto testFunDecl = []() {
        verifyDecl(val, ResVal<int>(Res::Err("Error")));
        return Res::Ok();
    };
    auto testFunCode = []() {
        verifyRes(Res::Err("Error"), 22, res.msg + " Code");
        return Res::Ok();
    };
    auto res = testFunRes();
    BOOST_CHECK_EQUAL(res.ok, false);
    BOOST_CHECK_EQUAL(res.msg, "Error");
    auto resVal = testFunDecl();
    BOOST_CHECK_EQUAL(resVal.ok, false);
    BOOST_CHECK_EQUAL(resVal.msg, "Error");
