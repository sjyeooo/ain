...
{
    verifyDecl(vault, GetVault(vaultId), "Vault <%s> not found", vaultId.GetHex());
...
Res CVaultView::SubVaultCollateral(const CVaultId& vaultId, CTokenAmount amount)
{
    ...
    verifyRes(amounts && amounts->Sub(amount), "Collateral for vault <%s> not found", vaultId.GetHex());
