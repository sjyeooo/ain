...
Res CICXOrderView::ICXCreateOrder(CICXOrderImpl const & order)
{
    ...
    verifyRes(!GetICXOrderByCreationTx(order.creationTx), "order with creation tx %s already exists!", order.creationTx.GetHex());
    verifyRes(order.orderType == CICXOrder::TYPE_INTERNAL || order.orderType == CICXOrder::TYPE_EXTERNAL, "invalid order type!");
    verifyRes(order.amountFrom != 0, "order amountFrom must be greater than 0!");
    verifyRes(order.amountToFill == order.amountFrom, "order amountToFill does not equal to amountFrom!");
    verifyRes(order.orderPrice != 0, "order price must be greater than 0!");
    verifyRes(order.expiry >= CICXOrder::DEFAULT_EXPIRY, "order expiry must be greater than %d!", CICXOrder::DEFAULT_EXPIRY - 1);
...
{
    //this should not happen, but for sure
    verifyRes(GetICXOrderByCreationTx(order.creationTx), "order with creation tx %s doesn't exists!", order.creationTx.GetHex());
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXMakeOfferByCreationTx(makeoffer.creationTx), "makeoffer with creation tx %s already exists!", makeoffer.creationTx.GetHex());
    verifyRes(makeoffer.amount != 0, "offer amount must be greater than 0!");
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXSubmitDFCHTLCByCreationTx(submitdfchtlc.creationTx), "submitdfchtlc with creation tx %s already exists!", submitdfchtlc.creationTx.GetHex());
    verifyRes(submitdfchtlc.amount != 0, "Invalid amount, must be greater than 0!");
    verifyRes(!submitdfchtlc.hash.IsNull(), "Invalid hash, htlc hash is empty and it must be set!");
    verifyRes(submitdfchtlc.timeout != 0, "Invalid timeout, must be greater than 0!");
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXSubmitEXTHTLCByCreationTx(submitexthtlc.creationTx), "submitexthtlc with creation tx %s already exists!", submitexthtlc.creationTx.GetHex());
    verifyRes(submitexthtlc.amount != 0, "Invalid amount, must be greater than 0!");
    verifyRes(!submitexthtlc.htlcscriptAddress.empty(), "Invalid htlcscriptAddress, htlcscriptAddress is empty and it must be set!");
    verifyRes(!submitexthtlc.hash.IsNull(), "Invalid hash, htlc hash is empty and it must be set!");
    verifyRes(submitexthtlc.ownerPubkey.IsFullyValid(), "Invalid refundPubkey is not a valid pubkey!");
    verifyRes(submitexthtlc.timeout != 0, "Invalid timout, must be greater than 0!");
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXClaimDFCHTLCByCreationTx(claimdfchtlc.creationTx), "claimdfchtlc with creation tx %s already exists!", claimdfchtlc.creationTx.GetHex());
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXCloseOrderByCreationTx(closeorder.creationTx), "closeorder with creation tx %s already exists!", closeorder.creationTx.GetHex());
...
{
    //this should not happen, but for sure
    verifyRes(!GetICXCloseOrderByCreationTx(closeoffer.creationTx), "closeooffer with creation tx %s already exists!", closeoffer.creationTx.GetHex());
