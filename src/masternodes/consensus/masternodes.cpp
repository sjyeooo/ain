...
Res CMasternodesConsensus::operator()(const CCreateMasterNodeMessage& obj) const {
    verifyRes(CheckMasternodeCreationTx());

    if (height >= static_cast<uint32_t>(consensus.EunosHeight))
        verifyRes(HasAuth(tx.vout[1].scriptPubKey), "masternode creation needs owner auth");
    ...
    if (height >= static_cast<uint32_t>(consensus.EunosPayaHeight)) {
        ...
        verifyRes(obj.timelock == 0, "collateral timelock cannot be set below EunosPaya");
    ...
    verifyRes(mnview.CreateMasternode(tx.GetHash(), node, obj.timelock));
...
Res CMasternodesConsensus::operator()(const CResignMasterNodeMessage& obj) const {
    verifyRes(HasCollateralAuth(obj));
...
Res CMasternodesConsensus::operator()(const CSetForcedRewardAddressMessage& obj) const {
    ...
    verifyRes(mnview.GetMasternode(obj.nodeId), "masternode %s does not exist", obj.nodeId.ToString());

    verifyRes(HasCollateralAuth(obj.nodeId), "%s: %s", obj.nodeId.ToString(), "tx must have at least one input from masternode owner");
...
Res CMasternodesConsensus::operator()(const CUpdateMasterNodeMessage& obj) const {
    ...
    verifyRes(HasCollateralAuth(obj.mnId));
