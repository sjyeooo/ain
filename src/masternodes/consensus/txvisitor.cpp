...
ResVal<CBalances> CCustomTxVisitor::BurntTokens() const {
    ...
    for (const auto& out : tx.vout)
        if (out.scriptPubKey.size() > 0 && out.scriptPubKey[0] == OP_RETURN)
            verifyRes(balances.Add(out.TokenAmount()));
...
ResVal<CBalances> CCustomTxVisitor::MintedTokens(uint32_t mintingOutputsStart) const {
    ...
    for (uint32_t i = mintingOutputsStart; i < tx.vout.size(); i++)
        verifyRes(balances.Add(tx.vout[i].TokenAmount()));
...
Res CCustomTxVisitor::HasCollateralAuth(const uint256& collateralTx) const {
    ...
    verifyRes(HasAuth(auth.out.scriptPubKey), "tx must have at least one input from the owner");
...
Res CCustomTxVisitor::TransferTokenBalance(DCT_ID id, CAmount amount, CScript const & from, CScript const & to) const {
    ...
    if (!from.empty())
        verifyRes(mnview.SubBalance(from, tokenAmount));

    // if "to" not supplied it will only sub balance from "form" address
    if (!to.empty())
        verifyRes(mnview.AddBalance(to,tokenAmount));
...
ResVal<CScript> CCustomTxVisitor::MintableToken(DCT_ID id, const CTokenImplementation& token) const {
    verifyRes(token.destructionTx == uint256{}, "token %s already destroyed at height %i by tx %s",
    ...
    if (static_cast<int>(height) < consensus.BayfrontHeight) {
        verifyRes(id >= CTokensView::DCT_ID_START, "token %s is a 'stable coin', can't mint stable coin!", id.ToString());

        verifyRes(HasAuth(auth.out.scriptPubKey), "tx must have at least one input from token owner");

        return {auth.out.scriptPubKey, Res::Ok()};
    }

    verifyRes(id != DCT_ID{0}, "can't mint default DFI coin!");

    verifyRes(!token.IsPoolShare(), "can't mint LPS token %s!", id.ToString());
    ...
    verifyRes(token.IsMintable() && (!isMainNet || !mnview.GetLoanTokenByID(id)), "token %s is not mintable!", id.ToString());
...
Res CCustomTxVisitor::EraseEmptyBalances(TAmounts& balances) const {
    for (auto it = balances.begin(), next_it = it; it != balances.end(); it = next_it) {
        ...
        verifyRes(mnview.GetToken(it->first), "reward token %d does not exist!", it->first.v);
    ...
    for (const auto& balance : balances) {
        ...
        if (token && token->IsPoolShare()) {
            ...
            if (bal.nValue == balance.second)
                verifyRes(mnview.SetShare(balance.first, owner, height));
        }
    }
    return Res::Ok();
}
...
Res CCustomTxVisitor::DelShares(const CScript& owner, const TAmounts& balances) const {
    for (const auto& kv : balances) {
        ...
        if (token && token->IsPoolShare()) {
            ...
            if (balance.nValue == 0)
                verifyRes(mnview.DelShare(kv.first, owner));
        }
    }
    return Res::Ok();
}
...
Res CCustomTxVisitor::SubBalanceDelShares(const CScript& owner, const CBalances& balance) const {
    ...
    verifyRes(mnview.SubBalances(owner, balance), CustomTxErrCodes::NotEnoughBalance, res.msg);
...
ResVal<CCollateralLoans> CCustomTxVisitor::CheckCollateralRatio(const CVaultId& vaultId, const CLoanSchemeData& scheme, const CBalances& collaterals, bool useNextPrice, bool requireLivePrice) const {
    ...
    verifyDecl(collateralsLoans, mnview.GetLoanCollaterals(vaultId, collaterals, height, time, useNextPrice, requireLivePrice));

    verifyRes(collateralsLoans->ratio() >= scheme.ratio, "Vault does not have enough collateralization ratio defined by loan scheme - %d < %d", collateralsLoans->ratio(), scheme.ratio);
...
Res CCustomTxVisitor::CheckNextCollateralRatio(const CVaultId& vaultId, const CLoanSchemeData& scheme, const CBalances& collaterals) const {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        verifyDecl(collateralsLoans, CheckCollateralRatio(vaultId, scheme, collaterals, useNextPrice, requireLivePrice));
        ...
        if (static_cast<int>(height) < consensus.FortCanningHillHeight)
            verifyRes(totalCollaterals >= collateralsLoans->totalCollaterals / 2, "At least 50%% of the collateral must be in DFI.");
        else
            verifyRes(arith_uint256(totalCollaterals) * 100 >= arith_uint256(collateralsLoans->totalLoans) * scheme.ratio / 2,
